#include "panels/TimeTrialRight.h"

#include <QSharedPointer>
#include <QSize>
#include <QWidget>
#include "panels/Interface.h"
#include "widgets/general_use/BackgroundRight.h"
#include "widgets/general_use/ScreenTitle.h"




UserInterface::Panel::TimeTrialRight::TimeTrialRight(UserInterface::PacketHandler* handler, QWidget* parent) :
    UserInterface::Panel::Interface(handler, parent) {
        
    m_background = new UserInterface::Widget::BackgroundRight(UserInterface::Widget::ID::Background, this);

    RegisterWidget(m_background);

}



void UserInterface::Panel::TimeTrialRight::ResizePanel(const QSize& newUsefulSize) {

    // call overridden function to resize background
    UserInterface::Panel::Interface::ResizePanel(newUsefulSize);

}



void UserInterface::Panel::TimeTrialRight::Startup(QSharedPointer<const Packet::Event::Interface> startInfo) {

    // TODO

}



void UserInterface::Panel::TimeTrialRight::Cleanup() {

    // TODO

}